...
#ifndef RT_GUI_ROS2_RT_GUI_CLIENT_H
#define RT_GUI_ROS2_RT_GUI_CLIENT_H

#include <rt_gui_ros2/support/client.h>
...
public:
  ...
  void sync()
  {
    ...
    {
      RCLCPP_WARN_ONCE(ros_node_->getNodePtr()->get_logger(),"RtGuiClient has not been initialized, please call the init() function before using sync().");
    }
  }

  void init(std::shared_ptr<rclcpp::Node> nh, const std::string server_name = RT_GUI_SERVER_NAME, const std::string client_name = RT_GUI_CLIENT_NAME, const double timeout = 30.0)
  {
    if(!ack_.joinable() && !requests_.joinable())
    {
      ...
      requests_ = std::thread(&RtGuiClient::_requests,this);
       ...
       RCLCPP_WARN(nh->get_logger(),"RtGuiClient has been already initialized! You are calling the same instance twice!",server_name.c_str());
    }
  }
...
private:
  ...
  void _init(std::shared_ptr<rclcpp::Node> nh, const std::string server_name, const std::string client_name, const double timeout = 30.0)
  {
    ...
    {
      RCLCPP_WARN(nh->get_logger(), "RtGuiClient could not find %s please select the right server_name when calling init().",server_name.c_str());
      init_ = false;
    }
  }
  ...
  bool loadFromServer(const std::string& group_name, const std::string& data_name, data_t& value)
  {
    ...
    {
      RCLCPP_WARN(ros_node_->getNodePtr()->get_logger(), "No initial value given in namespace %s",ns.c_str());
      res = false;
    }
    else
    {
