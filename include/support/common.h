...
//public:
//
//    enum sync_t {ASYNC,SYNC};
//
//    using callback_t = std::function<void(data_t)>;
//
//    Callback() {}
//
//    typedef std::tuple<callback_t,data_t,sync_t> value_t;
//    typedef std::map<key_t,value_t> callbacks_t;
//
//    void add(const std::string& key1, const std::string& key2, callback_t callback, data_t data, sync_t method = SYNC)
//    {
//        callbacks_[key_t(key1,key2)] = value_t(callback,data,method);
//    }
//
//    void update(const std::string& key1, const std::string& key2, const data_t& value)
//    {
//        if(std::get<2>(callbacks_[key_t(key1,key2)]) == SYNC) // Copy the new data in the buffer
//            std::get<1>(callbacks_[key_t(key1,key2)]) = value;
//        else // Call directly the callback with the new data
//            std::get<0>(callbacks_[key_t(key1,key2)])(value);
//    }
//
//    void sync()
//    {
//        for(auto tmp_map : callbacks_)
//        {
//            if(std::get<0>(tmp_map.second)!=nullptr) // The callback still exists
//                std::get<0>(tmp_map.second)(std::get<1>(tmp_map.second.second));
//        }
//    }
//
//private:
